...
## 1.7.0 — 2026-04-14
- 新增即時推送的訊息範本切換，在「顯示與通知」中可於 `Normal`（標準）與 `Minimal`（極簡）之間選擇。極簡範本將發送者與本文合併到第一行，時間下移到第二行；標準範本維持原本分行版面。GUI 端新增即時預覽面板，可直觀對比兩種版面。ID 顯示、時間格式、語言等既有自訂項目均疊加生效；舊設定檔缺少 `display.template` 鍵時自動回退為 `normal`，無需手動遷移。
