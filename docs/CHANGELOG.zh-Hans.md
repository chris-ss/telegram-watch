...
## 1.6.1 — 2026-03-21
- 为所有 SQLite 数据库（应用数据库和 Telethon session）启用 WAL 模式和 busy_timeout，提升云同步目录下的稳定性。新增 I/O 错误自动重试机制，doctor 命令检测到数据文件位于云同步目录时输出警告（REQ-20260321-001-sqlite-wal-retry）。
