...
## 1.7.0 — 2026-04-14
- 新增实时推送的消息模板切换，在「显示与通知」中可在 `Normal`（标准）与 `Minimal`（极简）之间选择。极简模板把发送者与正文合并到第一行，时间下移到第二行；标准模板维持原有分行版面。GUI 侧新增实时预览面板，可直观对比两种样式。ID 显示、时间格式、语言等既有定制项均叠加生效；老配置缺少 `display.template` 键时自动回退为 `normal`，无需手动迁移。
