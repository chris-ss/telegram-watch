...
## 1.7.0 — 2026-04-14
- リアルタイム送信メッセージのテンプレート切り替えを追加。「表示と通知」で `Normal`（標準）と `Minimal`（ミニマル）を選択でき、ミニマルは送信者名と本文を 1 行にまとめて時刻を 2 行目に送り、標準は従来の複数行レイアウトを維持します。GUI にライブプレビューを追加し、両テンプレートのレイアウトを一目で比較可能。ID 表示・時刻フォーマット・言語などの既存のカスタマイズは選んだテンプレートの上に適用され、`display.template` キーを持たない既存設定は自動的に `normal` にフォールバックします（移行作業は不要）。
